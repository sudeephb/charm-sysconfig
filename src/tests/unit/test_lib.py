...
class TestLib():
    ...
    def test_update_cpufreq(self, render, restart, config):
        ...
        sysh = lib_sysconfig.SysconfigHelper()
        sysh.update_cpufreq()
        render.assert_called_with(source=lib_sysconfig.CPUFREQUTILS_TMPL, target=lib_sysconfig.CPUFREQUTILS,
                                  templates_dir='templates', context=expected)

    @mock.patch('lib_sysconfig.hookenv.config')
    @mock.patch('lib_sysconfig.hookenv.log')
    @mock.patch('lib_sysconfig.render')
    def test_update_grub_file_true(self, render, log, config):
        config.return_value = {'reservation': 'isolcpus', 'cpu-range': '0-10', 'hugepages': '400', 'hugepagesz': '1G',
                               'config-flags': (
        ...
        render.assert_called_with(source=lib_sysconfig.GRUB_CONF_TMPL, target=lib_sysconfig.GRUB_CONF,
                                  templates_dir='templates', context=expected)
    ...
    def test_update_grub_file_false(self, render, log, config):
        config.return_value = {'hugepages': '400', 'hugepagesz': '1G',
                               'config-flags': (
                                   "{'grub': 'GRUB_DEFAULT=\"Advanced options for "
                                   "Ubuntu>Ubuntu, with Linux 4.15.0-38-generic\"'}"),
                               }
    ...
    def test_update_systemd_system_file_true(self, render, log, config):
        config.return_value = {'reservation': 'affinity', 'cpu-range': '0-10', 'hugepages': '400', 'hugepagesz': '1G',
        ...
        render.assert_called_with(source=lib_sysconfig.SYSTEMD_SYSTEM_TMPL, target=lib_sysconfig.SYSTEMD_SYSTEM,
                                  templates_dir='templates', context=expected)
